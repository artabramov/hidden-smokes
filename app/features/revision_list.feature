...
@revision @list
Scenario Outline: List revisions when datafile_id is not found
    # list revisions
Given set request header token from global param 'admin_token' 
 ...
 When send 'GET' request to url 'datafile/:datafile_id/revisions'
...
@revision @list
Scenario: List revisions when datafile_id is correct
    # list revisions
Given set request header token from global param 'admin_token' 
  And set request path param 'datafile_id' from global param 'datafile_id'
  And set request query param 'offset' from value '0'
